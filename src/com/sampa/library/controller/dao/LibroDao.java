...
import com.sampa.library.controller.mappers.LibroMapper;
import com.sampa.library.controller.mybatis.SqlMapFactory;
import com.sampa.library.models.pojos.Libro;

public class LibroDao implements LibroMapper, Serializable{

	private static final long serialVersionUID = 481302947534675487L;

	private static LibroMapper getMapper() {
		return SqlMapFactory.instance().getMapper(LibroMapper.class);
	}
...
//	}
	...
	public void insert(Libro libro) {
		...
		LibroDao.getMapper().insert(libro);
		SqlMapFactory.instance().closeSession();
	}

	@Override
	public List<DtoLibri> getAll() {
		List<DtoLibri> lista = new ArrayList<DtoLibri>();
		SqlMapFactory.instance().openSession();
		lista = LibroDao.getMapper().getAll();
		SqlMapFactory.instance().closeSession();
		return lista;
	}
	...
	public void delete(String isbn) {
		SqlMapFactory.instance().openSession();
		LibroDao.getMapper().delete(isbn);
		SqlMapFactory.instance().closeSession();
	...
	public void update(Libro libro) {
		SqlMapFactory.instance().openSession();
		LibroDao.getMapper().update(libro);
		SqlMapFactory.instance().closeSession();		
	}
	...
	public Libro getByTitle(String titolo) {
		SqlMapFactory.instance().openSession();
		Libro libro =LibroDao.getMapper().getByTitle(titolo);
		SqlMapFactory.instance().closeSession();
		return libro;
	}
}
